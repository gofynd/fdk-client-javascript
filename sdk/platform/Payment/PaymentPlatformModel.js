 ...
 * @property {AggregatorConfig[]} [aggregators] - List of all speceific Payment
 ...
 * @property {CartChargesSerializer[]} [cart_charges] - Contains all the charges
 *   that are applicable on cart
...
class PaymentPlatformModel {
  ...
  static GetRefundAccountResponse() {
    return Joi.object({
      is_verified_flag: Joi.boolean(),
      message: Joi.string().allow(""),
      data: Joi.array().items(PaymentPlatformModel.BankDetailsForOTP()),
      success: Joi.boolean().required(),
    });
  }
  ...
  static CartDetailsSerializer() {
    return Joi.object({
      ...
      cart_charges: Joi.array().items(
        PaymentPlatformModel.CartChargesSerializer()
      ),
