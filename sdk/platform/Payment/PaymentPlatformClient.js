...
class Payment {
  ...
  /**
   * @param {Object} arg - Arg object.
   * @returns {Promise<PayoutPennyDropAndChequePayload>} - Success response
   ...
   */
  async getPayoutPennyDropAndChequeConfig({} = {}) {
    const {
      error,
    ...
    if (warrning) {
      Logger({
        level: "WARN",
        message:
          "Parameter Validation warrnings for getPayoutPennyDropAndChequeConfig",
      });
      Logger({ level: "WARN", message: warrning });
    }
    ...
    const {
      error: res_error,
    } = PaymentModel.PayoutPennyDropAndChequePayload().validate(response, {
      abortEarly: false,
      allowUnknown: false,
    });
    ...
    if (res_error) {
      Logger({
        level: "WARN",
        message:
          "Response Validation Warnnings for getPayoutPennyDropAndChequeConfig",
      });
      Logger({ level: "WARN", message: res_error });
    }
