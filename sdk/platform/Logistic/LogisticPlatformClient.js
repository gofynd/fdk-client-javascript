...
const LogisticValidator = require("./LogisticPlatformValidator");
class Logistic {
  constructor(config) {
    this.config = config;
  }
  ...
  getEntityRegionView({ body } = {}) {
    const { error } = LogisticValidator.getEntityRegionView().validate(
      {
        body,
      },
  ...
  getListView({ pageNumber, pageSize, name, isActive, channelIds } = {}) {
    const { error } = LogisticValidator.getListView().validate(
      {
        pageNumber,
        pageSize,
        name,
   ...
   */
  getCompanyStoreView({} = {}) {
    const { error } = LogisticValidator.getCompanyStoreView().validate(
      {},
      { abortEarly: false, allowUnknown: true }
    );
    if (error) {
  ...
  getZoneDataView({ zoneId } = {}) {
    const { error } = LogisticValidator.getZoneDataView().validate(
      {
        zoneId,
      },
      { abortEarly: false, allowUnknown: true }
  ...
  updateZoneControllerView({ zoneId, body } = {}) {
    const { error } = LogisticValidator.updateZoneControllerView().validate(
      {
        zoneId,
        body,
      },
  ...
  insertZoneControllerView({ body } = {}) {
    const { error } = LogisticValidator.insertZoneControllerView().validate(
      {
        body,
      },
      { abortEarly: false, allowUnknown: true }
   ...
   */
  getStore({ storeUid } = {}) {
    const { error } = LogisticValidator.getStore().validate(
      {
        storeUid,
      },
      { abortEarly: false, allowUnknown: true }
   ...
   */
  getAllStores({} = {}) {
    const { error } = LogisticValidator.getAllStores().validate(
      {},
      { abortEarly: false, allowUnknown: true }
    );
    if (error) {
