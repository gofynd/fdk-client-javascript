...
class Theme {
  ...
  /**
   * @param {ThemePlatformApplicationValidator.CreateThemePreviewParam} arg - Arg object
   * @param {object} [arg.requestHeaders={}] - Request headers. Default is `{}`
   * @param {import("../PlatformAPIClient").Options} - Options
   * @returns {Promise<ThemePlatformModel.CreateThemePreviewResponseSchema>}
   *   - Success response
   *
   * @name createThemePreview
   * @summary: Create Theme Preview
   * @description: This endpoint creates the entry in redis to store data for theme preview. - Check out [method documentation](https://partners.fynd.com/help/docs/sdk/platform/theme/createThemePreview/).
   */
  async createThemePreview(
    { themeId, body, requestHeaders } = { requestHeaders: {} },
    { responseHeaders } = { responseHeaders: false }
  ) {
    const {
      error,
    } = ThemePlatformApplicationValidator.createThemePreview().validate(
      {
        themeId,
        body,
      },
    ...
    if (warrning) {
      Logger({
        level: "WARN",
        message: `Parameter Validation warrnings for platform > Theme > createThemePreview \n ${warrning}`,
      });
    }
    ...
    const response = await PlatformAPIClient.execute(
      this.config,
      "post",
      `/service/platform/theme/v2.0/company/${this.config.companyId}/application/${this.applicationId}/theme/${themeId}/preview`,
      query_params,
      body,
      requestHeaders,
      { responseHeaders }
    ...
    const {
      error: res_error,
    } = ThemePlatformModel.CreateThemePreviewResponseSchema().validate(
      responseData,
      { abortEarly: false, allowUnknown: true }
    );
    ...
    if (res_error) {
      if (this.config.options.strictResponseCheck === true) {
        ...
        Logger({
          level: "WARN",
          message: `Response Validation Warnings for platform > Theme > createThemePreview \n ${res_error}`,
        });
      }
    }
