...
const generateUrlWithParams = (item = {}, params) => {
  ...
  for (let key in params) {
    if (Array.isArray(params[key])) {
      let joinedParams = ''
      for (let index = 0; index < params[key].length; index++) {
        joinedParams += index === params[key].length - 1
          ? params[key][index]
          : params[key][index] + SLUG_DELIMETER;
      }
...
const findBestMatchingLink = (allLinks = [], pathname = '/') => {
  ...
  for (let i = 0; i < allLinks.length; i++) {
    ...
    if (linkArr.length === pathArr.length) // some match
    {
      let j;
      bestMatch = { value: '', params: {} };
      for (j = 0; j < linkArr.length; ++j) {
        if (linkArr[j].startsWith(':')) { // this will be :slug & test-slug
          let arrParams = [];
          if (pathArr[j].includes(SLUG_DELIMETER)) { // if link container multiple path params eg test-slug--test-slug-123
            arrParams = pathArr[j].split(SLUG_DELIMETER); // split it into array
          } else arrParams = [pathArr[j]] // else assign 
          if (bestMatch.params[linkArr[j].slice(1)]) { // if params is already present in best match object spread new params 
      ...
      if (j === linkArr.length) { // if all paths match and j is not 0 coz 'about-us' & 'notification' have same length and in that case j will be 0. This type of matching is handled in Regex on line #100  
        bestMatch.value = link
        break;
      }
    }
...
module.exports = { transformRequestOptions, getParamsFromItem, getQuery, trimChar, generateUrlWithParams, findBestMatchingLink, validURL, NAV_TYPE }